...
export async function GET(req: NextRequest) {
  ...
  // ── Profiles ──────────────────────────────────────────────────────────────
  const allProfiles: { id: string; name: string | null; created_at: string; survey_completed: boolean | null; survey_completed_at: string | null; group_name: string | null }[] = [];
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await admin
      .from('profiles')
      .select('id, name, created_at, survey_completed, survey_completed_at, group_name')
      .order('created_at')
    ...
    allProfiles.push(...(data as { id: string; name: string | null; created_at: string; survey_completed: boolean | null; survey_completed_at: string | null; group_name: string | null }[]));
